...
if (isset($_POST['login'])) {
    ...
    // Update the SQL query to check for 'isverify' = 1
    $query = mysqli_query($con, "SELECT ID, user_verify FROM tbluser WHERE (Email='$emailcon' OR MobileNumber='$emailcon') AND Password='$password'");
    $ret = mysqli_fetch_array($query);
    
    if ($ret > 0) {
        if ($ret['user_verify'] == 1) {
            $_SESSION['fosuid'] = $ret['ID'];
            header('location:index.php');
        } else {
           // $msg = "Your account is not verified. Please verify your email or contact support.";
           echo "<script> alert('Your account is not verified. Please verify your email or contact support.');</script>";
        }
    } else {
       // $msg = "Invalid Details.";
